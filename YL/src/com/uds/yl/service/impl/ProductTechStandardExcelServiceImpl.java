...
public class ProductTechStandardExcelServiceImpl implements IProductTechStandardExcelService{
	...
	public void getIndexBeanList(List<TCComponentBOMLine> materialIndexBomList) {
		...
		//创建一个全局sheet页
		File outFile = new File(Const.MaterialorAccIndexStandard.ENDPRO_INDEXSTANDARD_EXCEL_PATH);
		if (outFile.exists()) {
			outFile.delete();
		}
		// 设置边框
		...
		try {
			Runtime.getRuntime().exec("cmd /c start "+Const.MaterialorAccIndexStandard.ENDPRO_INDEXSTANDARD_EXCEL_PATH);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
