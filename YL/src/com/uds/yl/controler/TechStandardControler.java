...
import com.uds.yl.ui.MaterialStandardAndWaringSelectedFrame;
...
public class TechStandardControler implements BaseControler {
	private static TechStandarFrame frame ;
	...
	public int indexIntroduceIndex = 5;//指标说明下标
	public int methodIndex = 6;//检测方法的下标
	public int methodBasicIndex = 7;//检测方法依据下标
	public int remarkIndex = 8;//备注
	
	public int lawStartIndex = 9;//法规列的开始下标
	
//	 "指标名称","单位", "类型", "新内控标准","原内控标准","指标说明","检测方法","检测方法依据"
			...
			public void run() {
				try {
					if(frame==null || !frame.isVisible()){
						frame = new TechStandarFrame("质量技术标准生成器");	
					}
	...
	class TechStandarFrame extends JFrame  {
		...
		MaterialStandardAndWaringSelectedFrame materialStandardAndWaringSelectedFrame;
		 ...
		 * Create the frame.
		 */
		public TechStandarFrame(String name) {
			super(name);
			setBounds(100, 100, 1258, 635);
			...
			searchIndexBtn.setBounds(764, 90, 93, 25);
			...
			insertIndexBtn.setBounds(771, 248, 75, 25);
			...
			scrollPane_2.setBounds(49, 350, 1084, 146);
...
//						iTechStandarModifyServcie.getNewStatdard(itemRev, allTableBeans);
						...
						for (int i = 0; i < allTableBeans.size(); i++) {
							...
							indexModel.addRow(new String[] { 
									...
									bean.indexIntroduceString,//指标名称备注
							});
							
							indexModel.setValueAt(bean.remark, i, remarkIndex);//备注的值
			...
			indexTable.addMouseListener(new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent e) {
					...
					if(e.getClickCount()==2){
						...
						if(columnAtPoint==newStandardIndex || columnAtPoint==indexIntroduceIndex || columnAtPoint==methodBasicIndex || columnAtPoint==remarkIndex){//点击的是新内控标准
							callMaterialModifyUpAndDownJfreme(rawStandardStr, indexIntroduceStr, testGistStr,remarkStr);
						}
...
//			"指标名称","单位", "类型", "新内控标准","原内控标准","新预警值","原预警值","指标说明","检测方法","检测方法依据"
		 ...
		 * 内控标准和检测方法依据以及指标说明
		 */
		public void callMaterialModifyUpAndDownJfreme(String rawStandarStr,String introduceStr,String testGistStr,String remarkStr){
			if(materialStandardAndWaringSelectedFrame!=null&&materialStandardAndWaringSelectedFrame.isVisible()){
				return;
			}
			materialStandardAndWaringSelectedFrame = new MaterialStandardAndWaringSelectedFrame( rawStandarStr, introduceStr, testGistStr,remarkStr);
			materialStandardAndWaringSelectedFrame.setVisible(true);
			materialStandardAndWaringSelectedFrame.setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);
			materialStandardAndWaringSelectedFrame.setCallBack(new AbstractCallBack() {
				
				@Override
				public void setStandardAndWaringResult(String standardResult,
						String waringResult, String indexTesStr,
						String indexIntroduceStr,String indexRemarkStr) {
					DefaultTableModel model = (DefaultTableModel) indexTable.getModel();
					model.setValueAt(standardResult, rowAtPoint, newStandardIndex);//新内控
					model.setValueAt(indexTesStr, rowAtPoint,methodBasicIndex );//检测方法依据
					model.setValueAt(indexIntroduceStr, rowAtPoint, indexIntroduceIndex);//指标名称备注
					model.setValueAt(indexRemarkStr, rowAtPoint, remarkIndex);//备注
					
					//跟新到实体类里面
					TechStandarTableBean bean = allTableBeans.get(rowAtPoint);
					bean.newStandard = standardResult;
					bean.indexIntroduceString = indexIntroduceStr;
					bean.testGis = indexTesStr;
					bean.remark = indexRemarkStr;
