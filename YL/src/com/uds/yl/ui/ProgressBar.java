...
public class ProgressBar {
	 ...
	 * 
	 * @param title  进度条的名称
	 * @param thread  需要处理的线程
	 * @param parent  父窗口或者父窗口的边框数据，没有可为空，默认进度条显示在屏幕中央
	 */
	public ProgressBar(String title, Thread thread, Object parent){
		this.title = title;
		this.thread = thread;
		this.parent = parent;
	}
		...
		new Thread() {
			public void run() {
				try {
					...
					MessageBox.post("出错：\n\n" + e.getMessage() + "\n请联系管理员查看控制台输出的错误信息", "提示", MessageBox.INFORMATION);
					e.printStackTrace();					
				} finally {
					if (ui == null) {
						return;
