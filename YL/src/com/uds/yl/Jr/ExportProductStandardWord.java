...
public class ExportProductStandardWord {
	...
	public ExportProductStandardWord(TCComponentItemRevision mItemRev, String sessionName, String dataSetName) throws Exception {
...
//        String groupName = UserInfoSingleFactory.getInstance().getTCSession().getGroup().toString();
        ...
        builder.writeln(revisionProertiesBean.object_name + " 产品标准");
    	...
    	if (TablesBean1List.size() != 0) {
            ...
            for (int i = 0; i < revisionCount; i++) {
            	...
            	if (i == 0) {
            		...
            		builder.getCellFormat().setWidth(everyWidth/2);
            		builder.write("内控值");
            		
            		builder.insertCell();
            		builder.getCellFormat().setWidth(everyWidth/2);
            		builder.write("预警值");
            	} else if ( i == revisionCount - 1) {
            		builder.getCellFormat().setVerticalMerge(CellMerge.PREVIOUS);
                	builder.getCellFormat().setWidth(lastWidth);
                	builder.write("");
                } else {
            	...
            	for (int i = 0; i < revisionCount; i++) {
                	...
                	if (i == 0) {
                		...
                		if (tempBean == null) {
                			...
                			// 对于标准，内控值和预警值，要分不同的函数去获得
                			String indicatorStr = GetIndicatorStr(tempBean);
                			indicatorStr = indicatorStr.equals("") ? "-" : indicatorStr;
                			builder.write(indicatorStr);
                			
                			builder.insertCell();
                			String indicatorStr_1 = GetIndicatorStr_1(tempBean);
                			indicatorStr_1 = indicatorStr_1.equals("") ? "-" : indicatorStr_1;
                			builder.write(indicatorStr_1);
                		}
                	} else if ( i == revisionCount - 1) {
                    	builder.getCellFormat().setWidth(lastWidth);
                    	if (tempBean == null) {
                        	...
                        	String indicatorStr1 = GetIndicatorStr1(tempBean);
                			indicatorStr1 = indicatorStr1.equals("") ? "-" : indicatorStr1;
                        	builder.write(indicatorStr1);
                        }
                    } else {
                    	builder.getCellFormat().setWidth(everyWidth);
                    	if (tempBean == null) {
...
//        }
        ...
        for (BOMLinePropertiesBean bean : list) {
            ...
            // 根据规则构造预警值表达式
            builder.insertCell();
            builder.write(GetIndicatorStr_1(bean));
	...
	/**
	 * 用于产品标准表中表格获取预警值字符串</br>
	 * 使用的是U8_DOWNLINE、U8_DOWN_OPERATION、U8_UP_OPERATION、U8_UPLINE
	 */
	private String GetIndicatorStr_1(BOMLinePropertiesBean bean) {
		String cellStr = "";
		
		if (bean.bomline_U8_EARLYWARN_DOWNLINE.equals("") && bean.bomline_U8_EARLYWARN_UPLINE.equals("")) {
			cellStr = bean.bomline_U8_EARLYWARNDESC;
		} else if (bean.bomline_U8_EARLYWARN_DOWNLINE.equals("")) {
			if (bean.bomline_U8_EARLYWARNUP_OPT.equals("")) {
				cellStr = "<=  " + bean.bomline_U8_EARLYWARN_UPLINE;
			} else {
				cellStr = bean.bomline_U8_EARLYWARNUP_OPT + bean.bomline_U8_EARLYWARN_UPLINE;
			}
		} else if (bean.bomline_U8_EARLYWARN_UPLINE.equals("")) {
			if (bean.bomline_U8_EARLYWARN_DOWNOPT.equals("")) {
				cellStr = ">=  " + bean.bomline_U8_EARLYWARN_DOWNLINE;
			} else {
				cellStr = bean.bomline_U8_EARLYWARN_DOWNOPT + bean.bomline_U8_EARLYWARN_DOWNLINE;
			}		
		} else {
			if ((bean.bomline_U8_EARLYWARNUP_OPT.equals("") || bean.bomline_U8_EARLYWARNUP_OPT.equals("<="))
					&& (bean.bomline_U8_EARLYWARN_DOWNOPT.equals("") || bean.bomline_U8_EARLYWARN_DOWNOPT.equals(">="))) {
				cellStr = bean.bomline_U8_EARLYWARN_DOWNLINE + " - " + bean.bomline_U8_EARLYWARN_UPLINE;
			} else {
				cellStr = bean.bomline_U8_EARLYWARN_DOWNLINE + bean.bomline_U8_EARLYWARN_DOWNOPT
						+ " X "
						+ bean.bomline_U8_EARLYWARNUP_OPT + bean.bomline_U8_EARLYWARN_UPLINE;
			}
		}
